...
import { MapPin, Star, Heart, Clock, DollarSign, Music, ArrowLeft, Edit, ChevronLeft, ChevronRight, X, Tag } from 'lucide-react';
...
export default function VenueDetail() {
  ...
  // ✅ Cargar review existente del usuario cuando carga el detalle
  useEffect(() => {
  ...
  // ✅ Función para calificar — upsert para no duplicar
  const handleSubmitRating = async (selectedRating) => {
    if (!user) { alert('Inicia sesión para calificar'); return; }
    if (userRole === 'venue_admin') return; // admins no califican
  ...
  // Promedio calculado del lado del cliente como fallback
    ...
    <div className="min-h-screen bg-[#0a0a0a]">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          {/* ✅ SECCIÓN DE CALIFICACIÓN — solo para usuarios, no admins */}
          {user && userRole !== 'venue_admin' && (
            <div className="mb-6 p-4 bg-[#0f0f0f] border border-[#1a1a1a] rounded-xl">
              ...
              <div className="flex items-center space-x-2">
                ...
                {submittingRating && (
                  <div className="w-5 h-5 border-2 border-[#ff0080] border-t-transparent rounded-full animate-spin ml-2" />
                )}
                {userExistingReview && !submittingRating && (
                  <span className="text-xs text-gray-500 ml-2">Toca para cambiar</span>
                )}
              </div>
            </div>
          )}
