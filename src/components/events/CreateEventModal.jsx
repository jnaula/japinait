...
export default function CreateEventModal({ isOpen, onClose, onEventCreated }) {
  ...
  const handleImageUpload = async (e) => {
    try {
      ...
      // ✅ Bug 1 corregido: photo_url en lugar de photo_url_url
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            {venues.length === 0 && (
              <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                <p className="text-sm text-yellow-400">
                  No tienes locales aprobados. Debes tener al menos un local aprobado para crear eventos.
                </p>
              </div>
            )}
            ...
            <div>
              ...
              <div className="space-y-3">
                <label className="flex items-center justify-center w-full h-32 border-2 border-dashed border-[#2a2a2a] rounded-xl cursor-pointer hover:border-[#ff0080] transition-colors bg-[#1a1a1a]">
                  <div className="flex flex-col items-center gap-2">
                    {uploading ? (
                      <>
                        <div className="w-6 h-6 border-2 border-[#ff0080] border-t-transparent rounded-full animate-spin" />
                        <span className="text-sm text-yellow-400">Subiendo imagen...</span>
                      </>
                    ) : (
                      <>
                        <ImageIcon className="w-8 h-8 text-gray-600" />
                        <span className="text-sm text-gray-500">Toca para seleccionar imagen</span>
                      </>
                    )}
                  </div>
                ...
                {/* ✅ Bug 2 corregido: photo_url en lugar de im_url */}
                {formData.photo_url && (
                  <div className="relative w-full h-48 rounded-xl overflow-hidden border border-[#2a2a2a]">
                    ...
                    <div className="absolute bottom-2 left-2 px-2 py-1 bg-black/60 rounded-full">
                      <span className="text-xs text-green-400 font-medium">✓ Imagen lista</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="flex justify-end space-x-4 pt-4 border-t border-[#1a1a1a]">
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit"
                ...
                className="px-6 py-3 bg-gradient-to-r from-[#ff0080] to-[#7928ca] text-white rounded-lg font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
                {loading ? (
                  <>
