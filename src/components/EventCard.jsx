...
import { Calendar, MapPin, Clock, X, Heart, ChevronLeft, ChevronRight, ZoomIn } from 'lucide-react';
...
// ── Compartir usando Web Share API con fallback ────────────────────────────
async function shareEvent(event) {
  const imageUrl = event.photo_url || event.image_url || '';
  const text = `🎉 ${event.name}\n📍 ${event.venue_name || ''}\n🕐 ${new Date(event.event_date).toLocaleString('es-EC', { dateStyle: 'medium', timeStyle: 'short' })}`;

  // Web Share API (funciona en móvil nativo)
  if (navigator.share) {
    try {
  ...
  const waUrl = `https://wa.me/?text=${encodeURIComponent(text + (imageUrl ? `\n${imageUrl}` : ''))}`;
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            ...
            >
              {/* Hero imagen — clickeable para lightbox */}
              <div className="relative h-56 group cursor-zoom-in" onClick={() => imageUrl && setLightbox(true)}>
                {imageUrl ? (
                ...
                {/* Botones top */}
                <div className="absolute top-4 left-4 right-4 flex justify-between" onClick={(e) => e.stopPropagation()}>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-2 rounded-full bg-black/60 backdrop-blur-sm text-white text-xs font-semibold"
                    >
                      {/* Icono share manual para evitar import extra */}
              ...
              <div className="p-5">
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => shareEvent(event)}
                    className="flex-1 py-3 rounded-xl border border-[#2a2a2a] bg-[#161616] text-white font-bold text-sm flex items-center justify-center gap-2 hover:border-[#7928ca]/40 transition-colors"
                  >
                    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                      <circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/>
                      <line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/>
      ...
      <AnimatePresence>
        {lightbox && imageUrl && (
          ...
          >
            ...
            {/* Nombre del evento abajo */}
            <div className="absolute bottom-6 left-0 right-0 flex justify-center px-4">
              <div className="px-4 py-2 rounded-full bg-black/70 backdrop-blur-sm border border-white/10">
                <p className="text-white text-sm font-semibold text-center">{event.name}</p>
              </div>
            </div>
          </motion.div>
        )}
